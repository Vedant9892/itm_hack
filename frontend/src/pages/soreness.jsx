...
import {
  ...
  HiOutlineLightBulb, HiOutlineSparkles, HiOutlineCube,
} from 'react-icons/hi2';
import NeuralSidebar from '../components/dashboard/NeuralSidebar';
import MuscleModel3D from '../components/soreness/MuscleModel3D';
          ...
          {phase === 'select' && (
            <div className="h-full p-4 md:p-6 lg:p-8">
              <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_1fr] gap-6 h-full">
                <section className="relative overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-950 text-white min-h-[440px]">
                  <div className="absolute inset-0 pointer-events-none">
                    <div className="absolute -top-16 -left-10 h-52 w-52 rounded-full bg-indigo-500/20 blur-3xl" />
                    <div className="absolute bottom-8 right-8 h-40 w-40 rounded-full bg-emerald-400/10 blur-3xl" />
                  </div>

                  <div className="relative z-10 h-full p-5 md:p-6 flex flex-col gap-4">
                    <div className="flex items-center justify-between flex-wrap gap-3">
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-indigo-200/80">3D Muscle Selector</p>
                        <h2 className="text-xl md:text-2xl font-extrabold">Tap Pain Zones</h2>
                      </div>
                    ...
                    <div className="flex-1 min-h-[320px] rounded-[1.5rem] overflow-hidden border border-white/10">
                      <MuscleModel3D selectedMuscles={selected} onToggle={toggleMuscle} view={view} />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { k: 'View', v: view === 'front' ? 'Front anatomy' : 'Back anatomy' },
                        { k: 'Selected', v: `${selected.length} zone${selected.length === 1 ? '' : 's'}` },
                      ].map((item) => (
                        <div key={item.k} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3">
                          <p className="text-[10px] text-white/50 uppercase tracking-widest font-bold">{item.k}</p>
                          <p className="text-sm font-bold mt-0.5">{item.v}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <section className="rounded-[2rem] border border-slate-200 bg-white p-5 md:p-6 flex flex-col gap-5 min-h-[440px]">
                  <div>
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100 text-[11px] font-bold uppercase tracking-widest">
                      <HiOutlineCube className="text-sm" />
                      Recovery Builder
                    </div>
                    <h3 className="text-2xl font-extrabold text-slate-900 mt-3">Where are you sore?</h3>
                    <p className="text-sm text-slate-500 mt-1">Select all painful muscle groups, then generate a custom rehab and stretch protocol.</p>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 min-h-[90px]">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2">Selected Muscles</p>
                    <div className="flex flex-wrap gap-2">
                      {selected.length === 0
                        ? <p className="text-slate-400 text-sm italic">No muscles selected yet.</p>
                        : selected.map(id => (
                            <motion.span key={id} initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                              className="flex items-center gap-2 px-3.5 py-2 bg-rose-50 text-rose-700 rounded-full text-xs font-bold border border-rose-200"
                            >
                              {MUSCLE_LABELS[id] || id}
                              <button onClick={() => toggleMuscle(id)} className="hover:text-rose-500 transition-colors leading-none">×</button>
                            </motion.span>
                          ))}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      'Click muscle blocks in 3D',
                      'Switch between front and back',
                      'Generate AI recovery protocol',
                      'Track completed recovery tasks'
                    ].map((tip, i) => (
                  ...
                  <motion.button whileHover={{ scale: selected.length ? 1.01 : 1 }} whileTap={{ scale: 0.98 }}
                    onClick={handleGeneratePlan} disabled={!selected.length}
                    className={`mt-auto w-full py-4.5 rounded-2xl font-bold text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-3
                      ${selected.length
                        ? 'bg-gradient-to-r from-slate-900 to-indigo-900 text-white shadow-2xl shadow-indigo-900/25'
                        : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
                  >
                    <HiOutlineFire className={`text-xl ${selected.length ? 'text-emerald-400' : ''}`} />
                    Generate Recovery Plan ({selected.length})
                  </motion.button>
                </section>
              </div>
            </div>
